function session_hangup_hook(status)
  freeswitch.consoleLog("NOTICE", string.format("session_hangup_hook ==> UPDATE ptt_groups SET current_speaker_id = 0, start_time='2000-01-01 00:00:00' WHERE ptt_number=%s\n", ptt_number))
...
if(session:ready()) then
  ...
  db:query(string.format("SELECT ptt_members,current_speaker_id,start_time FROM ptt_groups WHERE ptt_number=%s", ptt_number), function(row)
    ...
    if(row.current_speaker_id ~= "0") then
      ...
      freeswitch.consoleLog("NOTICE", "Start Conference: " .. session:getVariavle("domain") .. "@default+flags{mute}")
    else
